...
resample_ks_test = function(d1, d2, min_size = FALSE, verbose = TRUE) {
  ...
  d1_sampled = sample(d1$d, size = n_muts, replace = FALSE)
  d2_sampled = sample(d2$d, size = n_muts, replace = FALSE)
  out = broom::tidy(
    suppressWarnings(ks.test(d1_sampled, d2_sampled))
  )
  return(out)
}
...
final_distances = muts_line %>% 
  ...
  bind_rows(
    ...
    adaptation_combined_distances %>% mutate(site_type = 'adaptation')
) %>% 
   mutate(
    site_type = case_when(
...
set.seed(42)
downsample_size = nrow(muts_line %>% filter(line == 5))
