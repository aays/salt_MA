...
props_all$salt_ma = salt_ma %>% 
  ...
  mutate(
    ...
    stderr = sqrt((numerator * (1 - numerator)) / callable_total)
  ) %>% 
  mutate(
    stderr = stderr / sum(stderr) # normalize standard errors
  )
...
get_props = function(d, d_callables, salt_ma_bool = FALSE) {
  ...
  out = out %>% 
    ...
    mutate(
      stderr = stderr / sum(stderr) # need to be multiplied by 2 for 95% stderr based on normal approximation
    )
  return(out)
}
...
Write combined salt MA bit to file for supplemental:

```{r}
perrineau_salt_ma_only = props_perrineau$salt_ma %>% 
  select(sample_factor, ant = annotation, prop) %>% 
  left_join(
    perrineau_cis$salt_ma, by = c('sample_factor', 'ant')
  )

# write_tsv(perrineau_salt_ma_only, here('data/supplemental/salt_ma_perrineau.tsv'))
```
...
names_y = c('ma_orig', 'adaptation', 'adaptation_silent', 'adaptation_nonsilent')
...
annotation_orig_plot = ma_orig_final %>% 
  bind_rows(adaptation_final) %>% 
  filter(gene_set != 'expressed genes') %>% 
  mutate(
    gene_set = factor(
      gene_set, levels = c('genome wide', 'salt genes')),
  ...
  scale_fill_manual(
    ...
    values = c(
      'MA' = palette[3], # '#87C5FC', orig color
      'salt-selection' = palette[5]) # '#FF5B5F', orig color
  ) +
  # macro elements
...
annotation_orig_plot

ggsave(here('plots/annotation_ma_hs.eps'), plot = annotation_orig_plot, width = 6, height = 4)
```
...
```{r}
fig_2_all = read_tsv(here('data/plots/fig_2_all.tsv'), col_types = cols())
palette = wes_palette(5, name = 'Darjeeling1', type = 'continuous')

annotation_plot_all = fig_2_all %>% 
  # set up data
  filter(gene_set != 'expressed genes', sample_factor != 'Salt MA (control)') %>% 
  ...
  mutate(
    ...
    annotation = fct_relevel(
      annotation, c('intergenic', "5' UTR", 'CDS', 'intronic', "3' UTR")
    )
  ) %>% 
  ...
  scale_fill_manual(
    values = c(
      'MA' = palette[1], # '#87C5FC', orig color
      'HS' = palette[3], # '#FF5B5F', orig color
...
ggsave(here('plots/annotation.eps'), plot = annotation_plot_all, width = 6, height = 4)
```
