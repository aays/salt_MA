...
annotation_plot_all = fig_2_all %>% 
  mutate(
    sample_factor = fct_relevel(sample_factor, 
      c('MA', 'HS', 'Salt MA (control)', 'Salt MA (salt)')),
    gene_set = fct_relevel(
      gene_set, c('genome wide', 'expressed genes', 'salt genes')),
    annotation = fct_relevel(
      annotation, c('intergenic', 'utr5', 'CDS', 'intronic', 'utr3')
    )
  ) %>% 
...
# ggsave(here('plots/annotation.eps'), plot = annotation_plot_all, width = 8, height = 4)
```
