...
# unlikely to hugely sway the results, but will need to correct for this down the line
...
rate_summarised$ma = make_final_count_table(count_table, ma_callables_long, 'ma') %>% 
  mutate(callables = callables * 940 * 6) %>%  # mean gens in these lines
  mutate(rate = count / callables)
rate_summarised$hs_silent = make_final_count_table(count_table, hs_callables_long_silent, 'adaptation_silent')
rate_summarised$hs_nonsilent = make_final_count_table(count_table, hs_callables_long_nonsilent, 'adaptation_nonsilent')
...
# TODO: an obs/exp kind of deal? would that not be the same plot as above though
