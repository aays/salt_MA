...
TODO: normalize relative to the total number of AT and GC sites in the genome! 

TODO: check orig script for stats on this - 2-tailed test

```{r}
get_eq_gc = function(d) {
  num_AT = 0.335 * 75417640 # these are the only numbers that make it fucking work
  num_GC = 0.665 * 75417640 # check against the silent site value for the adaptation dataset
  # num_AT = 0.365 * 6410499419
  # num_AT = d %>% filter(substr(mutation, 1, 1) %in% c('A', 'T')) %>% nrow()
  # num_GC = d %>% filter(substr(mutation, 1, 1) %in% c('G', 'C')) %>% nrow()
