...
salt_rate_summarised = salt_group_table %>% 
  ...
  mutate(rate = count / (callables * 250)) # sample count is baked in

salt_rate_summarised
# TODO: update site gen count once actual generation count is in
```
...
# salt MA - TODO, but currently setting to 250
rate_summarised$salt_0 = make_final_count_table(
  ...
  base_callables_summarised %>% 
    ...
    select(-sample_type) %>% 
    mutate(callables = callables * 250), # TODO: replace with exact value
  'salt_0')
...
# ggsave(here('plots/spectrum_rate_plot_rel.svg'), plot = spectrum_rate_plot_relative, width = 8, height = 4)
```
...
rate_summarised_combined %>% 
  ...
  mutate(
    type = str_replace(type, '>', ' \u2192 ')) %>% 
  mutate(
    rate = ifelse(sample_factor != 'MA', rate / 250, rate)
  ) %>% 
