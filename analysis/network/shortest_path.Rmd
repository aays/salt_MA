...
ma_resamples = read_tsv(
  here('data/network/resamples/ma_resample_100.tsv'),
  col_types = cols()
)
...
ggplot(all_resamples, aes(x = min_path)) +
  geom_histogram(
    binwidth = 1,
    color = 'black',
    fill = 'steelblue') +
...
resample_dist_plot = bind_rows(
  ...
  mutate(type = fct_relevel(type, c('HS genes', 'ChlamyNET resamples'))) %>% 
...
```{r}
ggplot(all_resamples %>% filter(iteration < 21), aes(x = min_path)) +
  geom_histogram(
    binwidth = 1,
    fill = 'steelblue',
    color = 'black'
...
all_resamples_binned = all_resamples %>% 
  group_by(iteration, min_path) %>% 
  summarise(random_count = n(), .groups = 'drop')

# get bin size 
resample_residuals = all_resamples_binned %>% 
  ...
  mutate(sq_diff = (random_count - adaptation_count)) %>% 
...
residual_plot = resample_residuals_summed %>% 
  ...
  scale_y_continuous(breaks = seq(-500, 750, 250)) +
  theme_classic() +
  labs(
    x = 'Minimum shortest path', 
    y = 'Random count - HS gene count',
    tag = 'B') +
  theme(
    axis.title = element_text(family = 'Helvetica', size = 12, color = 'black'),
    axis.text = element_text(family = 'Helvetica', size = 12, color = 'black'),
